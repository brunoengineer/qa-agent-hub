...
Apply QA Core Guidance, Documentation Output Guidance, Automation Guidance, and Jira QA Guidance from CLAUDE.md.
...
When `$ARGUMENTS` is provided, read the files at the given path and produce:
...
When (and only when) `$ARGUMENTS` is provided and you generate the report:
